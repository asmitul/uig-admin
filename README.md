...
2. `cd my-project && cp .env.example .env` to copy the example environment variables. You'll need to add the `DATABASE_URI` from your Cloud project to your `.env` if you want to use S3 storage and the MongoDB database that was created for you.
...
- Modify the `DATABASE_URI` in your `.env` file to `mongodb://127.0.0.1/<dbname>`
- Ensure the `DATABASE_URI` reference in `docker-compose.yml` matches the above `<dbname>`
